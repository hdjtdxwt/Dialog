...
public class ProgressView extends View {
    private static final float OUTER_LAYER_LARGE_SCALE = 28 / 32.F;
    private static final float MIDDLE_LAYER_LARGE_SCALE = 28 / 32.F;
    private static final float SHADOW_LAYER_LARGE_SCALE = 20 / 32.F;
    private static final float SMALL_CIRCLE_DEGREE_OFFSET = 0.7F;

    private static final int DEF_VIEW_SIZE = 150;
    ...
    public ProgressView(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        TypedArray typedArray = context.getTheme().obtainStyledAttributes(attrs, R.styleable.ProgressView, defStyleAttr, R.style.def_progress_style);
        int indexCount = typedArray.getIndexCount();
        for (int i = 0; i < indexCount; i++) {
            int attr = typedArray.getIndex(i);
            switch (attr) {
                case R.styleable.ProgressView_outer_layer_solide_color:
                    outLayerSolideColor = typedArray.getColor(attr, Color.BLACK);
                    break;
                case R.styleable.ProgressView_outer_layer_stroke_color:
                    outLayerStrokeColor = typedArray.getColor(attr, Color.BLACK);
                    break;
                case R.styleable.ProgressView_outer_layer_stroke_width:
                    outLayerStrokeWidth = typedArray.getDimensionPixelOffset(attr, 0);
                    break;
                case R.styleable.ProgressView_midlle_layer_progress_color:
                    midlleLayerProgressColor = typedArray.getColor(attr, Color.BLACK);
                    break;
                case R.styleable.ProgressView_midlle_layer_progress_width:
                    midlleLayerProgressWidth = typedArray.getDimensionPixelOffset(attr, 0);
                    break;
                case R.styleable.ProgressView_midlle_layer_bg_color:
                    midlleLayerBgColor = typedArray.getColor(attr, Color.BLACK);
                    break;
                case R.styleable.ProgressView_midlle_layer_small_circle_solide_color:
                    smallCircleSolideColor = typedArray.getColor(attr, Color.BLACK);
                    break;
                case R.styleable.ProgressView_midlle_layer_small_circle_stroke_color:
                    smallCircleStrokeColor = typedArray.getColor(attr, Color.BLACK);
                    break;
                case R.styleable.ProgressView_midlle_layer_small_circle_size:
                    smallCircleSize = typedArray.getDimensionPixelSize(attr, 0);
                    break;
                case R.styleable.ProgressView_midlle_layer_small_circle_stroke_width:
                    smallCircleStrokeWidth = typedArray.getDimensionPixelOffset(attr, 0);
                    break;
                case R.styleable.ProgressView_shadow_layer_color:
                    shadowLayerColor = typedArray.getColor(attr, Color.BLACK);
                    break;
                case R.styleable.ProgressView_shadow_layer_inner_color:
                    shadowLayerInnerColor = typedArray.getColor(attr, Color.BLACK);
                    break;
                case R.styleable.ProgressView_inner_text_size:
                    innerTextSize = typedArray.getDimensionPixelSize(attr, 0);
                    break;
                case R.styleable.ProgressView_inner_text_color:
    ...
    private Point getTextPointInView(String textDesc) {
        ...
        point.set(textW, viewSize / 2 + textH / 2 - 10);
        return point;
    }
        ...
        Observable.interval(0, 1, TimeUnit.SECONDS)
                ...
                .subscribe(new Subscriber<Long>() {
                    ...
                    public void onNext(Long aLong) {
                        ...
                        --countdownTime;
                        if (countdownTime < 0) {
                            mTextDescPaint.setTextSize(innerTextSize *2/3);
                            progressDesc = "时间到";
                            onCompleted();
                            return;
                        } else {
